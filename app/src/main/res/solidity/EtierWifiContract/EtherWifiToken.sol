...
contract EtherWifiToken is mortal {
    /**
    ...
    function getAccessPoint(string _macAddress) constant returns(string, address, uint, bool) {
        return (accessPoints[_macAddress].password, accessPoints[_macAddress].sharer, accessPoints[_macAddress].count, accessPoints[_macAddress].isShared);
    }
    ...
    function addCount(string _macAddress, uint _num) {
        accessPoints[_macAddress].count += _num;
    }
